...
for order in CONFIG_ORDER:
    ...
    print(f'{cid.nicename_nomt_noft()} & {time:.0f} & {events:.2f} & {timeperevent:.0f} \\\\')

print('\\hline')

print(f'Total & {average_sane(total_time, method=np.median):.0f} & {average_sane(total_events, limitL=1, limitR=1000):.2f} & {average_sane_timeperevent(total_timeperevent, total_events, limitL=1, limitR=1000):.0f} \\\\')
